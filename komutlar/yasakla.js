...
module.exports.run = async (bot, message, args) => {
  ...
  if (!message.member.hasPermission("ADMINISTRATOR"))
   if (!message.member.roles.has("YETKİLİ ID")) {
...
//CODE WORLD BY DEVOLOPER CANROW
...
//CODE WORLD BY DEVOLOPER CANROW
  const embed = new Discord.RichEmbed()
    .setColor("BLACK")
    .setDescription(`${u} Adlı kişinin yasaklanmasını onaylıyormusunuz?`)
    .setFooter(bot.user.username, bot.user.avatarURL);
  message.channel.send(embed).then(async function(sentEmbed) {
    ...
    reactions.on("collect", async function(reaction) {
      if (reaction.emoji.name === "✅") {
        let log = message.guild.channels.find(`name`, "ban-log");  
        message.channel.send(`İşlem Onaylandı! ${u} Adlı Kişi Sunucudan Yasaklandı!`);
