...
exports.run = async (client, message, args) => {
  ...
  if (opponent.id === message.author.id) return message.reply('Kendin ile düello atamazsın!');
		...
		try {
			if (!opponent.bot) {
                await message.channel.send(`${opponent}, düello isteği geldi. <a:ONAYLANDI:756422581287911434> Düello'yu kabul ediyor musun? (\`evet\` veya \`hayır\` olarak cevap veriniz.)`);
